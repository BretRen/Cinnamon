...
class ContactDetailsRepository(
    ...
    fun fetchLatestContactDetails(contactId: Long) = context.contentResolver.observe(ContactsContract.Data.CONTENT_URI).map {
    ...
    suspend fun toggleFavorite(
        ...
        val favorite = if (isFavorite) 0 else 1
        val values = contentValuesOf(
            ContactsContract.Data.STARRED to favorite
        )

        context.contentResolver.update(ContactsContract.Contacts.CONTENT_URI, values, "${ContactsContract.Contacts._ID} = ?", arrayOf(contactId.toString()))
    }
}